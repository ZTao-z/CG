...
public:
	...
	// 画直线
	void drawLine(GLint Xa, GLint Ya,
				  GLint Xb, GLint Yb,
		          std::vector<GLfloat>& res) {
		drawLineWithBresenham(Xa, Ya, Xb, Yb, res);
	}
...
private:
	...
	const int WINDOW_HEIGHT;
public:
	Bezier(int w, int h) :WINDOW_WIDTH(w), WINDOW_HEIGHT(h) {};

	// 保存直线上的点
	void savePoint(float temp_X, float temp_Y, std::vector<GLfloat>& out) {
		GLfloat color = 1.0f;
		out.push_back(GLfloat(float(temp_X) / float(WINDOW_WIDTH / 2)) - 1.0f);
		out.push_back(1.0f - GLfloat(float(temp_Y) / float(WINDOW_HEIGHT / 2)));
		out.push_back(0.0f);
	...
	void drawBezier(std::deque<Point>& points, std::vector<GLfloat>& out) {
		if (points.size() <= 2) {
			return;
		}
		for (int t = 0; t <= 1000; t++) {
			...
			for (int i = 0; i < points.size(); i++) {
				x += C(points.size()-1, i) * pow(0.001*t, i) * pow((1.0 - 0.001*t), points.size()-1 - i) * points[i].x;
				y += C(points.size()-1, i) * pow(0.001*t, i) * pow((1.0 - 0.001*t), points.size()-1 - i) * points[i].y;
			}
			savePoint(x, y, out);
		}
	}

	void show(std::deque<Point>& points, float t) {
		if (points.size() <= 1) {
			return;
		}
		std::deque<Point> v;
		for (int i = 0; i < points.size()-1; i++) {
			Point temp(int(t * points[i+1].x + (1 - t) * (points[i].x) + 0.5), int(t * points[i+1].y + (1 - t) * (points[i].y) + 0.5));
			v.push_back(temp);
		}
		points = v;
	}
